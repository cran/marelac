...
c result              : Absolute Salinity anomaly          [g/kg]
        ...
        double precision p0, p0_original, longs0, lats0, sa_upper
        double precision sa_lower, delta
        double precision longs(nx), lats(ny), p(nz), del_sa(nz,ny,nx)
        double precision dlongs, dlats
        double precision r1, s1, t1, dsa(4), ndepth(ny,nx), ndepth_max
        logical ISNAN
...
c result              : absolute salinity anomaly of data        [g/kg]
       ...
       double precision dsa_add(4), dsa(4), dsa_mean
       double precision longs_pan(6), lats_pan(6), longs0, lats0, r
       double precision lats_line
       double precision longs, lats, dlongs, dlats
...
c result        : nanmean of the 4 adjacent neighbours     [g/kg]
        ...
        integer k, nmean
        double precision dsa(4)
        double precision dsa_mean, dsa_add(4)
...
c result              : interpolated value
        ...
        implicit none
        integer n, k
        double precision x(n), y(n), x0, r, xinterp1
